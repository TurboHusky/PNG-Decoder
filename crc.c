...
void deinterlace(uint8_t *buf, uint32_t width, uint32_t height)
{
...
void load_bytes (uint8_t *buf, int len, int *index, union dbuf *input, uint32_t *shift)
{
   uint32_t loop = *shift >> 3;
   if (loop && *index < len - 4)
   {
...
void decompress_dynamic(uint8_t *buf, int len, uint32_t width, uint32_t height)
{
   struct huf_build
   {
      uint32_t count;
   ...
   while(alphabet_code_count<HLIT+HDIST)
   {
      ...
      load_bytes(buf, len, &index, &input, &shift);
   }
   ...
   while(index<(len-4))
   {
      load_bytes(buf, len, &index, &input, &shift); // FIXME: Worst case ~ 48 bits consumed
      ...
      for(int bits=1; bits<=ALPHABET_SIZE; bits++)
      {
         ...
         if(literal_length_code<literal_length_limits[bits])
         {
            ...
            {
               // Literal
               printf("%02X ", literal_length_index);
            }
            ...
            else if(literal_length_index <286)
            {
               ...
               printf("%d:%d ", tmp.value, tmp.extra);
               ...
               for(int distance_bit_count=1; distance_bit_count<=ALPHABET_SIZE; distance_bit_count++)
               {
                  ...
                  if(distance_code<distance_limits[distance_bit_count])
                  {
                     ...
                     printf("%d:%d ", dist_tmp.value, dist_tmp.extra);
...
// DYNAMIC HUFFMAN
   ...
   decompress_dynamic(dynamic_huffman, sizeof(dynamic_huffman), 20, 24);
   return 0;
}